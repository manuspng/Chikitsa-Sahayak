...
export function printClinicalReport(record: Partial<AnalysisRecord>) {
  let activeLogo = logoImg;
  try {
    const savedLogo = localStorage.getItem("app_logo_url");
    if (savedLogo) {
      activeLogo = savedLogo;
    }
  } catch (e) {
    console.warn("Could not read custom logo from localStorage", e);
  }

  const finalLogoUrl = activeLogo.startsWith("data:") || activeLogo.startsWith("http")
    ? activeLogo
