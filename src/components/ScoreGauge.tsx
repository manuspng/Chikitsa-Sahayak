...
export default function ScoreGauge({ score, maxScore, label, riskLevel }: ScoreGaugeProps) {
  ...
  return (
    <div className="bento-card border-2 border-slate-300 flex flex-col md:flex-row md:items-center justify-between gap-6 transition-all duration-300">
      <div className="flex-1">
        <h4 className="card-title font-mono font-black text-xs uppercase tracking-wider mb-1">{label}</h4>
        <div className="flex items-baseline gap-2">
          <div className="score-big leading-none select-none tracking-tighter">
            {score.toFixed(1).endsWith(".0") ? score : score.toFixed(1)}
            <span className="text-xl font-black font-sans tracking-normal ml-1">/ {maxScore}</span>
          </div>
        </div>
        <p className="text-xs font-bold flex items-center gap-2 mt-2">
      ...
      <div className="w-full md:w-64 flex flex-col justify-end">
        <div className="text-xs font-black uppercase tracking-wider mb-2 text-right">
          Trend Progress bar
        </div>
        ...
        <div className="flex justify-between text-xs font-mono mt-2 font-black select-none">
          <span>0.0</span>
